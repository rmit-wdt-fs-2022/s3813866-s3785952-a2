...
using Assignment2Models.Utility;

namespace Assignment2Models.Models;
...
public class Customer
{
    ...
    [ModelAttributes.IsAName("^([a-zA-Z]{2,}\\s[a-zA-Z]{1,}'?-?[a-zA-Z]{2,}\\s?([a-zA-Z]{1,})?)", ErrorMessage = "Name Must Have The Valid Characters of (A-Z) (a-z) (' space -)") ]
    ...
    [ModelAttributes.IsAPostCode(@"^(0[289][0-9]{2})|([1345689][0-9]{3})|(2[0-8][0-9]{2})|(290[0-9])|(291[0-4])|(7[0-4][0-9]{2})|(7[8-9][0-9]{2})$", ErrorMessage = "Post Code is Not Valid")]
    [StringLength(4)]
    public string? PostCode { get; set; }
    
    [Phone]
    [ModelAttributes.IsAPhoneNumber(@"^(?:\+?(61))? ?(?:\((?=.*\)))?(0?[2-57-8])\)? ?(\d\d(?:[- ](?=\d{3})|(?!\d\d[- ]?\d[- ]))\d\d[- ]?\d[- ]?\d{3})$", ErrorMessage = "Phone number is not valid")]
    [StringLength(12)]
